...
| `sms_slider.lua` | Reusable horizontal slider — track + draggable handle + editable value box, two-way synced (drag OR type). `M.new(parent, {initial,min,max,step,decimals,value_w,tooltip,on_change})`; pure value↔pixel math in `M._math`. Deliberately hand-rolled (not dxgui's native `Slider`) on the `splitter`/`clearable_edit` pattern. Used by Paint Statics' radius/density/spacing/heading rows. |
| `sms_grid.lua` | Shared sortable-grid plumbing for tool windows: `build_grid(opts)` (skinned `Grid` + optional `sms_scrollbars`), `wire_sortable_headers(grid, cols, opts)` (builds header cells + owns the asc/desc toggle via `get_sort`/`on_sort`/`is_sortable` callbacks), `update_header_labels(headers, key, dir)` (▲/▼ re-text). Each window keeps its own comparator, row model, and `onMouseDown`. Used by `prefab_manager.lua`, `mass_edit.lua`, `trigger_finder.lua`. |
...
| `static_preview_panel.lua` | Embedded 3D model preview for Paint Statics — wraps ED's `DemoSceneWidget` with drag-rotate / wheel-zoom. Loads our **private** scene `scenes/sms_static_preview_scene.lua` (not DCS's shared `staticPreview.lua`) so the viewport never contends with the vanilla ME Static panel over the single global camera slot; falls back to the shared scene if the private one can't load. pcall-isolated; degrades to a metadata caption without blocking the tool. |
| `scenes/sms_static_preview_scene.lua` | DCS DemoScene script (NOT a require-able module — loaded by path via `DemoSceneWidget:loadScript`). A clone of DCS's `Scripts/DemoScenes/staticPreview.lua` with the global state table renamed to `dcs_sms_static_preview`, giving our 3D preview a private camera independent of the vanilla Static panel's. |
...
| `trigger_finder.lua` | The Trigger Finder tool window (`sms_window`): left Grid tree of the current map selection (groups → units, statics and trigger zones as leaves) with per-node trigger-count badges, right pane of one Button per referencing trigger. Per-frame `selection.snapshot()` poll auto-follows selection; clicking a trigger drives the native `me_trigrules` listbox (`selectItem`+`setItemVisible`+`onChange`) to select it, opening the panel via the toolbar toggle if closed. |
...
| `community_tab.lua` | The Community tab UI in the Prefab Manager (browse/search/sort/tags/detail/import + the right-column image-preview panel: lazy screenshot fetch via a dedicated `W.media_job` slot, disk cache, `◀ i / N ▶` navigation, aspect-correct fit-to-column). Also renders a yellow missing-mod warning line above the import button via `prefab_modules.absent` — shown only when the selected entry needs a mod that isn't installed. |
