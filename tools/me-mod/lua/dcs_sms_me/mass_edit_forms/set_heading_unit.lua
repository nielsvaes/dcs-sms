...
-- ---------------------------------------------------------------------------
-- Undo handler -- shared between Absolute and Delta. Each row carries the
-- old heading in RADIANS (captured before mutation); we convert to degrees
-- and re-apply via the same verb so its side effects (psi sync, map redraw)
-- fire on undo too.
-- ---------------------------------------------------------------------------
...
local function form_height()
    local L = LAYOUT
    return 2 * L.ROW_H + L.GAP_Y + L.FOOTER_PAD
end
