-- mass_edit_forms/add_suffix_unit_name.lua -- Mass Edit form: append a
-- literal string to every selected unit's name via verbs.unit_set_name.
--
-- Unit-scope sibling of add_suffix_group_name. Wraps mass_edit_transforms
-- .add_suffix (plain concatenation; with keep_num=true, the suffix is
-- inserted BEFORE a trailing `-<digits>` / `_<digits>` block so
-- "Viper-1" + "X" becomes "ViperX-1"). Universal applicability -- every
-- unit category accepts a rename. Collisions (Mission.renameUnit refusal)
-- are counted as failed; unchanged rows (transform yielded same string)
-- are silently skipped, matching the group-scope form.
--
-- Public:
--   M.scope   : 'unit'
--   M.title   : 'Add suffix to unit names'
--   M.new(parent_raw, get_checked, on_after_apply, get_categories)
--   M._apply(entities, args, categories)
--             args = { text = string, keep_num = boolean? }
...
local function log_warn(msg)
    pcall(function() _G.log.write('sms.me.mass_edit.add_suffix_unit_name', _G.log.WARNING or 2, msg) end)
end
...
function M._apply(entities, args, categories)
    ...
    if type(args) ~= 'table' or type(args.text) ~= 'string' or args.text == '' then
        return {
            changed = 0, failed = 0, not_applicable = 0, changed_rows = {},
            toast = 'Enter a suffix', sev = 'warning',
        }
    end
    ...
    for idx, u in ipairs(entities) do
        local old = u.name
        local new = transforms.add_suffix(old, { text = args.text, keep_num = args.keep_num == true }, idx)
        if new == old then
            -- silent skip (matches group-scope add_suffix_group_name)
        else
    ...
    if #changed_rows > 0 then
        undo.record_generic('mass_edit.add_suffix_unit_name', { rows = changed_rows })
    end
    ...
    if #changed_rows == 0 and failed == 0 then
        ...
        result.toast = toast
        result.sev   = (failed == 0 and 'success') or (#changed_rows == 0 and 'error') or 'warning'
    end
...
-- with the OLD name. Collisions on restore are counted as partial failures.
-- ---------------------------------------------------------------------------

undo.register_handler('mass_edit.add_suffix_unit_name', function(snapshot)
    if type(snapshot) ~= 'table' or type(snapshot.rows) ~= 'table' then
        return nil, 'invalid mass_edit.add_suffix_unit_name undo snapshot'
    end
    local verbs = require('dcs_sms_me.verbs')
    local errors = 0
        ...
        if ok and t then
            ...
            if t.setTooltipText then
                pcall(t.setTooltipText, t,
                    ...
                    '(e.g. "Viper-1" + "X" -> "ViperX-1").')
            end
            -- Default ON: the keep-num-aware insert is what users want
            -- almost every time on DCS-named units (Viper-1, etc.).
    ...
    if apply_btn and apply_btn.addMouseDownCallback then
        pcall(apply_btn.addMouseDownCallback, apply_btn, function()
            pcall(function()
                ...
                local result   = M._apply(entities, { text = text, keep_num = keep_num }, cats)
