...
M.applies_to = { ground = true, ship = true, static = true }
...
local KM_TO_M = 1000
local MI_TO_M = 1852 -- nautical miles; DCS uses NM for unqualified mission distances
local MAX_RADIUS = 1000 -- km / NM
...
function M._apply(entities, radius, unit)
    ...
    if type(radius) ~= 'number' or radius ~= radius or radius == math.huge or radius == -math.huge or radius < 0 or radius > MAX_RADIUS then
        return {
            changed = 0, failed = 0, changed_rows = {},
            toast = 'Enter a finite radius from 0 to ' .. MAX_RADIUS, sev = 'warning',
        }
    end
        ...
        pcall(apply_btn.addMouseDownCallback, apply_btn, function()
            pcall(function()
                ...
                local entities = (type(get_checked) == 'function')
                                 and get_checked() or {}
                local result = M._apply(entities, radius, unit, txt)
