-- Standalone test for mass_edit_forms.add_suffix_group_name.
...
local form = require('dcs_sms_me.mass_edit_forms.add_suffix_group_name')
...
-- Case 3: single happy. Foo + "-Y" → "Foo-Y".
do
    undo.clear()
    mock.new_mission()
    renames = {}
    local g = mock.add_plane({ name = 'Foo' })
    local result = form._apply({ g }, '-Y')
    check('single happy: changed=1', result.changed == 1)
    check('single happy: failed=0',  result.failed == 0)
    check('single happy: g.name = Foo-Y', g.name == 'Foo-Y')
    check('single happy: renameGroup called once', #renames == 1)
    check('single happy: toast = "1 suffixed"',
          result.toast == '1 suffixed', 'got ' .. tostring(result.toast))
...
do
    ...
    check('multi: g1 = Alpha-TEST',   g1.name == 'Alpha-TEST')
    check('multi: g2 = Bravo-TEST',   g2.name == 'Bravo-TEST')
    check('multi: g3 = Charlie-TEST', g3.name == 'Charlie-TEST')
    check('multi: toast = "3 suffixed"', result.toast == '3 suffixed')
    check('multi: sev = success', result.sev == 'success')
    check('multi: undo recorded', undo.has_record() == true)
end

-- Case 5: undo restores prior names.
do
    local ok = undo.undo()
...
-- Case 8: module metadata
do
    check('form.scope = group', form.scope == 'group')
    check('form.title = Add suffix to group names',
...
print('All add_suffix_group_name tests passed.')
