...
func meWaypointRemoveTaskFlags() (*flag.FlagSet, *meWaypointRemoveTaskOpts) {
	opts := &meWaypointRemoveTaskOpts{}
	fs := flag.NewFlagSet("me waypoint remove-task", flag.ContinueOnError)
...
func meWaypointRemoveTaskCmd(args []string, stdout, stderr io.Writer) int {
	fs, opts := meWaypointRemoveTaskFlags()
	fs.SetOutput(stderr)
	...
	if hasName == hasID {
		fmt.Fprintln(stderr, "dcs-sms me waypoint remove-task: exactly one of --group-name or --group-id is required")
		return 2
	}
	if !opts.indexSet || opts.Index < 0 {
		fmt.Fprintln(stderr, "dcs-sms me waypoint remove-task: --index is required (integer >= 0)")
		return 2
	}
	if !opts.slotSet || opts.Slot < 1 {
		fmt.Fprintln(stderr, "dcs-sms me waypoint remove-task: --slot is required (integer >= 1)")
		return 2
	}
	var idClause string
	if hasName {
	...
	resp, exitCode := runMeVerb("waypoint_remove_task", luaArgs, opts.Timeout, opts.SavedGames, stderr)
	if exitCode != 0 {
		return exitCode
	}
