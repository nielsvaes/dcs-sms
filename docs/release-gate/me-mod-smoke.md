...
- [ ] **Image previews (me-mod ≥ 0.25.0):** select a prefab that has screenshots (e.g. `snow-city`) → the WHOLE image appears on the right, below the description, scaled to fit the column (not a cropped corner). With >1 image, `◀ i / N ▶` shows and the arrows cycle (wrapping). Drag the splitter / resize the window → the image rescales and keeps its aspect ratio (no stretching). Select a prefab with no images → the panel collapses and the description fills the column. With `lib/` removed (no networking), selecting a prefab shows no image and does not error.
- [ ] **Enlarge window (me-mod ≥ 0.25.0):** click the thumbnail → a larger, resizable window opens showing the same image scaled to fit; resize it → the image re-fits (aspect preserved). Its `◀ ▶` cycle the images and stay in sync with the inline thumbnail's counter. Selecting a different prefab in the tab hides the window; switching to another tab (My Prefabs / Triggers) also closes it. Clicking the thumbnail again re-opens it.
- [ ] **No show-through on resize (regression, 0.25.1):** on the Community tab, select a prefab so its image preview (and, for a mod-dependent entry, the yellow missing-mod warning) is visible → switch to **My Prefabs** → **resize the window**. The Community image/warning must NOT appear over the My Prefabs page. Repeat ending on the **Triggers** tab (should also stay clean).
